...
static struct sock_with_lock* get_unix_sock_comp_channel(int comp_channel_fd)
{
...
void connect_router(struct sock_with_lock *unix_sock)
{
    ...
    const char* path = "/router/";
    const char* router_name = getenv("ROUTER_NAME");
    ...
    if (connect(unix_sock->sock, (struct sockaddr*)&saun, len) < 0) {
